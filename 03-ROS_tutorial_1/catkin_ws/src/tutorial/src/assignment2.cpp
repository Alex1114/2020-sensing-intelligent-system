...
tf::Matrix3x3 R1(0, -1, 0, 1, 0, 0, 0, 0, 1),
              ...
              R3(0.75, -0.2165, 0.625, 0.433, 0.875, -0.2165, -0.5, 0.433, 0.75),
...
void broadcastTF(const ros::TimerEvent& timer_event){
  ...
  br.sendTransform(tf::StampedTransform(T4*T3.inverse()// Which transform should be used here?
                                        , ros::Time::now(), "C", "E"));
  br.sendTransform(tf::StampedTransform(T3*T4.inverse()*T2.inverse()*T5// Which transform should be used here? 
                                        , ros::Time::now(), "E", "F"));
}
